...
public class HealingCalculator {
    ...
    public static void healingTick(Player p) {
        ...
        double regenAmount = stats.getMaxHealth() * 0.02 * (1 + (stats.getStat(Stat.VITALITY, true) / 100));

        if(stats.getActiveHealth() + regenAmount <= stats.getMaxHealth()) {
            stats.setActiveHealth(stats.getActiveHealth() + regenAmount);
        }else{
            stats.setActiveHealth(stats.getMaxHealth());
        }
